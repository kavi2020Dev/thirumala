 ...
 const Lanuage = i18n.language;
  return (
    <div className='flex flex-col items-center' style={{zIndex:'9'}}>
     <Avatar variant='square' src={ThiruMala} sx={{width:'150px', height:'auto', my:'3px', mb:Lanuage == 'en' ? '20px' : '3px'}}/>
     <Avatar variant='square' src={Lanuage == 'en' ? English : Tamil} sx={{width:isRes ? '85%' : '60%', height:'auto', my:'3px'}}/>
     <Avatar variant='square' src={Lanuage == 'en' ? Seng : Stamil} sx={{width:isRes ? '55%' : '40%', height:'auto', mt:Lanuage == 'en' ?'15px' : '3px'}}/>
     <Avatar variant='square' src={Logo} sx={{width:isRes ? '90%' :'410px', height:'auto', position:'absolute', bottom:'0px', pt:'10px', px:'10px'}}/>
    </div>
  )
}
